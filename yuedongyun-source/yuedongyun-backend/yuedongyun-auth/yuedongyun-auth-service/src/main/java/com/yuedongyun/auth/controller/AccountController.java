package com.yuedongyun.auth.controller;
...
public class AccountController {
    ...
    public String refreshToken(
            @CookieValue(value = JwtConstants.REFRESH_HEADER, required = false) String memberToken,
            @CookieValue(value = JwtConstants.ADMIN_REFRESH_HEADER, required = false) String adminToken
    ) {
        if (memberToken == null && adminToken == null) {
        ...
        String token = host.startsWith("www", 7) ? memberToken : adminToken;
        if (token == null) {
            throw new BadRequestException("登录超时");
        }
