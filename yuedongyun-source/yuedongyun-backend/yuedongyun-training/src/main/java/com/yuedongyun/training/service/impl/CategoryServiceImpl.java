...
import com.yuedongyun.common.constants.Constant;
import com.yuedongyun.common.constants.ErrorInfo;
import com.yuedongyun.common.enums.CommonStatus;
...
import com.yuedongyun.training.mapper.CategoryMapper;
import com.yuedongyun.training.mapper.AssessmentCategoryMapper;
...
import com.yuedongyun.training.utils.CategoryDataWrapper;
import com.yuedongyun.training.utils.CategoryDataWrapper2;
...
public class CategoryServiceImpl extends ServiceImpl<CategoryMapper, Category> implements ICategoryService {
    ...
    public void add(CategoryAddDTO categoryAddDTO) {
        ...
        int level = 1; //默认一级分类
        if (TrainingConstants.CATEGORY_ROOT != categoryAddDTO.getParentId()) {
            //校验父分类是否存在
            Category parentCategory = this.baseMapper.selectById(categoryAddDTO.getParentId());
            if (parentCategory == null) {
                throw new BizIllegalException(TrainingErrorInfo.Msg.CATEGORY_PARENT_NOT_FOUND);
            }
            //三级训练分类下不能在创建子分类
            if (parentCategory.getLevel() == 3) {
                throw new BizIllegalException(TrainingErrorInfo.Msg.CATEGORY_CREATE_ON_THIRD);
            }
            //分类级别，父分类+1
            level = parentCategory.getLevel() + 1;
        }
        if (level > 3) {
            throw new BizIllegalException(TrainingErrorInfo.Msg.CATEGORY_ADD_OVER_THIRD_LEVEL);
        }
    ...
    public void delete(Long id) {
        ...
        if (CollectionUtil.isNotEmpty(categories)) { //分类有子分类
            throw new BizIllegalException(TrainingErrorInfo.Msg.CATEGORY_HAVE_CHILD);
        }
        //2.查询分类信息
        Category category = this.baseMapper.selectById(id);
        //2.1.判空
        ...
        //3.统计分类拥有的训练数量
        Integer trainingNum = trainingService.countTrainingNumOfCategory(id);
        //3.1.分类拥有训练数据量判空
        if (trainingNum > 0) {
            throw new BizIllegalException(TrainingErrorInfo.Msg.CATEGORY_DELETE_HAVE_TRAINING);
        }
        //4.统计分类拥有的题目数量
        int assessmentNum = assessmentCategoryMapper.countAssessmentNum(category.getId(), category.getLevel());
        //4.1.分类拥有的题目数量判空
        if (assessmentNum > 0) { //训练含有题目
            throw new BizIllegalException(TrainingErrorInfo.Msg.CATEGORY_DELETE_HAVE_ASSESSMENT);
        }
        //5.删除训练
        int result = this.baseMapper.deleteById(id);
        if (result <= 0) {
            throw new DbException(TrainingErrorInfo.Msg.CATEGORY_DELETE_FAILD);
        }
    }

    /**
     * 功能点：
     * 1.启用或禁用训练，下一级或下一级的训练同时启用或禁用，
     * 联动启用或禁用
     *
    ...
    public void disableOrEnable(CategoryDisableOrEnableDTO categoryDisableOrEnableDTO) {
        ...
        if (category == null) {
            throw new BizIllegalException(TrainingErrorInfo.Msg.TRAINING_CATEGORY_NOT_FOUND);
        }
        //2.校验
        if (category.getParentId() != 0) { //校验父级分类
            ...
            if (categoryDisableOrEnableDTO.getStatus() == CommonStatus.ENABLE.getValue()) {
                ...
                if (parentCategory == null) {
                    log.error("操作异常，根据父类id查询训练分类未查询到，parentId : {}", category.getParentId());
                    throw new BizIllegalException(ErrorInfo.Msg.OPERATE_FAILED);
                }
                //2.3父分类禁用下不能启用当前分类
                if (CommonStatus.DISABLE.getValue() == parentCategory.getStatus()) {
                    throw new BizIllegalException(TrainingErrorInfo.Msg.CATEGORY_ENABLE_CANNOT);
                }
            }
        }
        ...
        if (categoryDisableOrEnableDTO.getStatus() == CommonStatus.DISABLE.getValue()) {
            ...
            taskExecutor.execute(() -> {
                batchDownShelfTraining(category.getId(), category.getLevel(), userId);
            });
    ...
    public void update(CategoryUpdateDTO categoryUpdateDTO) {
        ...
        if (category == null) {
            throw new BizIllegalException(TrainingErrorInfo.Msg.CATEGORY_NOT_FOUND);
        }
        //2.校验名称是否可以更新
        checkSameName(category.getParentId(), categoryUpdateDTO.getName(), categoryUpdateDTO.getId());
        //3.设置更新字段
        Category updateCategory = new Category();
        updateCategory.setId(categoryUpdateDTO.getId()); //修改训练分类id
    ...
    public List<SimpleCategoryVO> all(Boolean admin) {
        ...
        List<Long> categoryIdList = admin ?
                null :trainingService.getCategoryIdListWithTraining();
        // 1.1.判空
    ...
    @Override
    public List<String> queryTrainingCategorys(Training training) {
        //1.查询训练分类
        List<Category> categories = baseMapper.selectBatchIds(
                Arrays.asList(training.getFirstCateId(),
                        training.getSecondCateId(),
                        training.getThirdCateId()));
        ...
        Map<Long, String> categoryIdAndNameMap = categories.stream()
                .collect(Collectors.toMap(Category::getId, Category::getName));
        //2.按照分类层级关系组装成列表
        return Arrays.asList(categoryIdAndNameMap.get(training.getFirstCateId()),
                categoryIdAndNameMap.get(training.getSecondCateId()),
                categoryIdAndNameMap.get(training.getThirdCateId()));
    }
    ...
    private void checkSameName(Long parentId, String name, Long currentId) {
        ...
        if (currentId == null && CollectionUtil.isNotEmpty(categories)) {
            throw new BizIllegalException(TrainingErrorInfo.Msg.CATEGORY_SAME_NAME);
        }
    ...
    private void batchDownShelfTraining(Long categoryId, Integer level, Long userId) {
        //1.多线程下设置操作用户id
        UserContext.setUser(userId);
        //2.查询需要下架的训练
        List<Training> trainings = trainingService.queryByCategoryIdAndLevel(categoryId, level);
        ...
        for (Training training : trainings) {
            //4.判断状态是否可以下架
            if (!TrainingStatus.SHELF.equals(training.getStatus())) {
                continue;
            }
            try {
                //5.训练下架
                trainingDraftService.downShelf(training.getId());
            } catch (Exception e) {
                log.error("训练下架异常");
            }
        }
