...
import com.yuedongyun.common.exceptions.ForbiddenException;
import com.yuedongyun.common.utils.*;
import com.yuedongyun.media.constants.FileErrorInfo;
...
import com.yuedongyun.media.mapper.MediaMapper;
import com.yuedongyun.media.service.IMediaService;
import com.yuedongyun.media.storage.IMediaStorage;
...
import static com.yuedongyun.media.constants.FileErrorInfo.MEDIA_NOT_EXISTS;
...
public class MediaServiceImpl extends ServiceImpl<MediaMapper, Media> implements IMediaService {
    ...
    @Override
    public VideoPlayVO getPlaySignatureBySessionId(Long sessionId) {
        // 1.根据sessionId查询媒训练信息
        SessionInfoDTO sessionInfo = trainingClient.sessionInfo(sessionId);
        Long trainingId = sessionInfo.getTrainingId();
        // 2.查询用户训练计划，是否是购买过的训练
        Long sessionId = workoutClient.isSessionValid(trainingId);

        if(sessionId != null){
            // 2.1.是，查询媒资信息，直接获取签名
            Media media = getById(sessionInfo.getMediaId());
        ...
        // 2.2.否，判断训练章节是否免费
        Boolean trailer = sessionInfo.getTrailer();
        ...
        String signature =  mediaStorage.getPlaySignature(
                media.getFileId(), UserContext.getUser(), sessionInfo.getFreeDuration());
        // 5.返回
        VideoPlayVO vo = new VideoPlayVO();
        vo.setSignature(signature);
        vo.setFileId(media.getFileId());
    ...
    public PageDTO<MediaVO> queryMediaPage(MediaQuery query) {
        ...
        createIds.remove(0L);
        // 4.查询引用次数
        List<MediaQuoteDTO> mediaQuoteDTOS = trainingClient.mediaUserInfo(ids);
