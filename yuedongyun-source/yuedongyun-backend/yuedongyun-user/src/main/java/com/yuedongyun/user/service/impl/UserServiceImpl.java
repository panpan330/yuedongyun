...
import com.yuedongyun.api.dto.user.LoginFormDTO;
import com.yuedongyun.api.dto.user.UserDTO;
import com.yuedongyun.common.domain.dto.LoginUserDTO;
...
import com.yuedongyun.common.exceptions.UnauthorizedException;
import com.yuedongyun.common.utils.AssertUtils;
...
import com.yuedongyun.user.domain.dto.UserFormDTO;
import com.yuedongyun.user.domain.po.User;
import com.yuedongyun.user.domain.po.UserDetail;
import com.yuedongyun.user.domain.vo.UserDetailVO;
import com.yuedongyun.user.enums.UserStatus;
import com.yuedongyun.user.mapper.UserMapper;
import com.yuedongyun.user.service.ICodeService;
...
import static com.yuedongyun.user.constants.UserConstants.*;
import static com.yuedongyun.user.constants.UserErrorInfo.Msg.*;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    public LoginUserDTO queryUserDetail(LoginFormDTO loginDTO, boolean isStaff) {
        ...
        // 5.判断用户类型与登录方式是否匹配
        if (isStaff ^ user.getType() != UserType.MEMBER) {
            throw new BadRequestException(isStaff ? "非管理端用户" : "非会员端用户");
        }
        // 6.封装返回
    ...
    public UserDetailVO myInfo() {
        ...
        switch (type) {
            case STAFF:
                ...
                break;
            case MEMBER:
                vo.setRoleName(MEMBER_ROLE_NAME);
                break;
            case COACH:
                vo.setRoleName(COACH_ROLE_NAME);
                break;
            default:
                break;
        }
    ...
    public Long saveUser(UserDTO userDTO) {
        ...
        if(type == UserType.COACH){
            detail.setRoleId(COACH_ROLE_ID);
        }else{
