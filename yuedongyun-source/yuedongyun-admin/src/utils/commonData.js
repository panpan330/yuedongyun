// 教练状态数据
export const statusData = [
  {
    value: 1,
    label: "正常",
  },
  {
    value: 0,
    label: "禁用",
  },
]
// 难易程度数据
export const difficultyData = [
  {
    value: 1,
    label: "简单",
  },
  {
    value: 2,
    label: "中等",
  },
  {
    value: 3,
    label: "困难",
  },
]
// 训练难度数据
export const trainingDifficultyData = [
  {
    value: 1,
    label: "入门",
  },
  {
    value: 2,
    label: "进阶",
  },
  {
    value: 3,
    label: "高阶",
  },
]
// 題目類型
export const titleTypeData = [
  {
    value: 1,
    label: "单选题",
  },
  {
    value: 2,
    label: "多选题",
  },
  {
    value: 3,
    label: "不定向选择",
  },
  {
    value: 4,
    label: "判断题",
  }
]
// 答案类型
export const titleAnswerData = [
  {
    value: 1,
    label: "正确",
  },
  {
    value: 0,
    label: "错误",
  }
]
// 售卖模式
export const sellingModelData = [
  {
    value: 0,
    label: "免费",
  },
  {
    value: 1,
    label: "付费",
  }
]
// 训练状态
export const trainingStatusData = [
  {
    value: 1,
    label: "待上架",
  },
  {
    value: 2,
    label: "已上架",
  },
  {
    value: 3,
    label: "已下架",
  },
  {
    value: 4,
    label: "已完结",
  }
]
// 步骤条数据
export const stepData = [
  {
    value: 1,
    label: "基本信息",
  },
  {
    value: 2,
    label: "训练目录",
  },
  {
    value: 3,
    label: "训练视频",
  },
  {
    value: 4,
    label: "训练考核",
  },
  {
    value: 5,
    label: "训练教练",
  }
]
// 优惠卷类型数据
export const vouchersTypeData = [
  {
    value: 1,
    label: "每满减",
  },
  {
    value: 4,
    label: "满减",
  },
  {
    value: 2,
    label: "折扣",
  },
  {
    value: 3,
    label: "无门槛",
  },
]
// 优惠券状态数据
export const vouchersStatusData = [
  {
    value: 1,
    label: "待发放",
  },
  {
    value: 2,
    label: "未开始",
  },
  {
    value: 3,
    label: "进行中",
  },
  {
    value: 4,
    label: "已结束",
  },
  {
    value: 5,
    label: "已暂停",
  },
]
// 优惠券使用范围数据
export const vouchersScopeData = [
  {
    value: 0,
    label: "不限定",
  },
  {
    value: 1,
    label: "指定训练分类",
  }
]
// 优惠券推广方式数据
export const vouchersWayData = [
  {
    value: 1,
    label: "手动领取",
  },
  {
    value: 2,
    label: "指定发放",
  }
]
// 优惠券发放数量数据
export const vouchersGrantData = [
  {
    value: 1,
    label: "无限量",
  },
  {
    value: 2,
    label: "指定数量",
  }
]

// 订单状态数据
export const orderStatusData = [
  {
    value: 1,
    label: "未支付",
  },
  {
    value: 2,
    label: "已支付",
  },
  {
    value: 3,
    label: "已关闭",
  },
  {
    value: 4,
    label: "已完成",
  },
  {
    value: 5,
    label: "已报名",
  },
  // {
  //   value: 6,
  //   label: "已退款",
  // }
]
// 退款状态数据
export const refundStatusData = [
  {
    value: 1,
    label: "待审批",
  },
  {
    value: 2,
    label: "会员取消退款",
  },
  {
    value: 3,
    label: "同意退款",
  },
  {
    value: 4,
    label: "拒绝退款",
  },
  {
    value: 5,
    label: "退款成功",
  },
  {
    value: 6,
    label: "退款失败",
  }
]
// 支付方式数据
export const payChannelCodeData = [
  {
    value: 'weixinPay',
    label: "微信",
  },
  {
    value: 'aliPay',
    label: "支付宝",
  },
  // {
  //   value: 'simulate',
  //   label: "模拟通道",
  // }
]
// 支付方式数据
export const resultData = [
  {
    value: 1,
    label: "同意",
  },
  {
    value: 2,
    label: "拒绝",
  }
]
// 问题状态
export const questionsData = [
  {
    value: 1,
    label: "已查看",
  },
  {
    value: 0,
    label: "未查看",
  }
]
// 性别
export const sexData = [
  {
    value: 1,
    label: "男",
  },
  {
    value: 2,
    label: "女",
  }
]
// 优惠券发放方式
export const grantTypeData = [
  {
    value: 1,
    label: "立刻发放",
  },
  {
    value: 2,
    label: "定时发放",
  }
]
// 优惠券发放方式
export const grantDeadlineData = [
  {
    value: 1,
    label: "固定天数",
  },
  {
    value: 2,
    label: "固定时间段",
  }
]
// 兑换码
export const redeemTypeData = [
  {
    value: 1,
    label: "未兑换",
  },
  {
    value: 2,
    label: "已兑换",
  }
]
// 兑换码
export const showStautsData = [
  {
    value: 0,
    label: "显示",
  },
  {
    value: 1,
    label: "隐藏",
  }
]

