...
export function validatePhone (rule, value, callback) {
  ...
  if (value === "" || value === undefined || value === null) {
    if (rule.required) {
      callback("教练手机号为空，请输入教练手机号")
    } else {
      callback()
    }
  } else {
    if (!reg.test(value) && value !== "") {
      callback("教练手机号格式错误，请重新输入")
    } else {
      callback()
    }
  }
...
// 会员手机校验
export function validatemembersPhone (rule, value, callback) {
  const reg = /^(13[0-9]|14[01456879]|15[0-35-9]|16[2567]|17[0-8]|18[0-9]|19[0-35-9])\d{8}$/
  if (value === "" || value === undefined || value === null) {
    if (rule.required) {
      callback("会员手机号为空，请输入会员手机号")
    } else {
      callback()
    }
  } else {
    if (!reg.test(value) && value !== "") {
      callback("会员手机号格式错误，请重新输入")
    } else {
      callback()
    }
  }
...
export const validateContacts = (rule, value, callback) => {
  ...
  if (value === "" || value === undefined || value == null) {
    callback(new Error("教练名称为空，请输入教练名称"))
  } else {
    if (reg.test(value)) {
      callback(new Error("教练名称格式错误，请输入中、英文"))
    } else {
      callback()
    }
  }
...
// 会员
export const validatemembers = (rule, value, callback) => {
  const reg = /[^\a-\z\A-\Z\u4E00-\u9FA5]$/g
  if (value === "" || value === undefined || value == null) {
    callback(new Error("会员名称为空，请输入会员名称"))
  } else {
    callback()
  }
}
...
export const validatorPrice = (rule, value, callback) => {
  if (!value) {
    return callback(new Error("训练价格为空，请输入训练价格"))
  }
  if (value > 100000 || value <= 0) {
    callback(new Error("训练价格需在0.01～100000.00元，请重新输入"))
  }
  if (!Number(value)) {
    callback(new Error("训练价格格式错误，请重新输入"))
  } else {
    const reg = /(^\d+\.?\d{0,2}$)/
...
export const validatorValidDuration = (rule, value, callback) => {
  if (!value) {
    return callback(new Error("训练有效期为空，请设置训练有效期"))
  }
  first(value, callback)
  const re = /(^[1-9]\d*$)/
  const rsCheck = re.test(value)
...
export const validatorTime = (rule, value, callback, time, text) => {
  if (!value) {
    callback(new Error("领取期限为空，请设置领取期限"))
  } else {
