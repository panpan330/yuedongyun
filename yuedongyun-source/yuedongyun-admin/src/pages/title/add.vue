        ...
        >
          ...
          <el-form-item label="所属训练：">
            <div class="el-input">
              ...
              >
                <el-option
                  v-for="item in trainingData.value"
                  :key="item.id"
                  :label="item.name"
                  :value="item.id"
                ></el-option>
          ...
          <el-form-item label="题目类型：" prop="type">
            <el-radio-group v-model="fromData.type" @change="handleAssessmentType">
              <el-radio
...
// 接口api
import { getDetails, addAssessment, editAssessment, checkName } from "@/api/title";
import { getTypeAll, getTrainingsAll } from "@/api/api";
// 公用数据
import {
  difficultyData,
  titleTypeData,
...
let trainingData = reactive([]); //根据题目分类获取所属训练
let trainingIds = reactive([]); //所属训练ids
let trainingId = ref(null); //当前标题id
let tinymces = ref([]);
...
const getDetailData = async () => {
  await getDetails(trainingId.value)
    .then((res) => {
      if (res.code === 200) {
        ...
        // 根据题目三级分类获取所属训练
        getTraining(thirdCateId);
        // 处理所属训练
        fromData.value.trainingIds = res.data.trainingIds ? res.data.trainingIds : [];
        trainingIds.push(fromData.value.trainingIds);
      }
    })
    .catch((err) => {});
};
...
const handleSubmit = async (str) => {
  ...
  if (valid) {
    ...
    baseVal.trainingIds = trainingIds;
    // 处理训练分类数据，提交的时候只传1-3级的id
    // let thirdCateId = [];
    // if (trainingId.value !== "null") {
    ...
    let params = {
      ...
      trainingIds: baseVal.trainingIds,
    ...
    let func = !baseVal.id ? addAssessment : editAssessment;
    await func(params)
      .then((res) => {
        if (res.code === 200) {
          ElMessage({
...
// 根据题目三级分类获取所属训练
const getTraining = async (ids) => {
  await getTrainingsAll(ids)
    .then((res) => {
      if (res.code === 200) {
