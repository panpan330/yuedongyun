...
// 接口api
import { getAssessmentPage, getDetails } from "@/api/title"
// 导入组件
// 新增按钮
...
const getList = async () => {
  loading.value = true
  let {assessmentTypes , ... params} = searchData;
  params.types = assessmentTypes;
  await getAssessmentPage(params)
    .then((res) => {
      if (res.code === 200) {
        loading.value = false
...
const handleCheckAllChange = (val) => {
  ...
  if (val) {
    ...
    checkedCities.value = []
    searchData.assessmentTypes = null
  }
...
const handleCheckedCitiesChange = (value) => {
  ...
  searchData.assessmentTypes = types.join(",")
  getList()
}
//紧录入我的
