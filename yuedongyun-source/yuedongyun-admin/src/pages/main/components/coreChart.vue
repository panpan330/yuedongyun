...
const legendData = ["访问量", "订单金额", "订单笔数", "新增会员"]
const selectedData =  ref({ '订单金额': true, '订单笔数': true, '新增会员': false, '访问量': false })

// 订单金额所取的y轴，内容为数字0
const yAxisIndex = ref({amount: 0, count: 1, newMembers: 1,Visits: 1})
// 创建一个erchart柱状图
  ...
  const options = {
    tooltip: {
      ...
      formatter: function (params) {
        ...
        res += '<div style="font-family:PingFangSC-Regular;font-size: 12px;color: #332929;letter - spacing: -0.2px;width:236px;margin-top:5px;margin-bottom:15px">' + `<span style="width: 5px;height: 5px;background: ${color} ;display:inline-block;border-radius: 50%;overflow:hidden;;margin-bottom:4px;margin-right:7px"> + </span>` + params[i].seriesName + ' : ' + params[i].value  +'<br/>' + `<span style="display:flex;justify-content: space-between;margin-top:6px;"><span>峰值:${params[i].seriesName == legendData[0] ? arr.maxVisits : params[i].seriesName == legendData[1] ? arr.maxamount : params[i].seriesName == legendData[2] ? arr.maxcount : params[i].seriesName == legendData[3] ? arr.maxnewMembers : ''}</span><span>谷值:${params[i].seriesName == legendData[0] ? arr.minVisits : params[i].seriesName == legendData[1] ? arr.minamount : params[i].seriesName == legendData[2] ? arr.mincount : params[i].seriesName == legendData[3] ? arr.minnewMembers : ''}</span></span>`
          + '</div>'+ '</div>'
        }
        return res
      }
...
const setEcharts = (params, arrTag, type) => {
    ...
    // 重置Y轴对应关系 并配置对应的数据
    // dataArr 数据
    let sl = { '订单金额': false, '订单笔数': false, '新增会员': false, '访问量': false }
  ...
  await getGrantInfo(params)
    .then(res => {
      if (res.code === 200) {
        ...
        watchArr.newMembers.series = res.data.series[2]
        watchArr.newMembers.xaxis = res.data.xaxis[0]
        watchArr.newMembers.yaxis = res.data.yaxis[2]
