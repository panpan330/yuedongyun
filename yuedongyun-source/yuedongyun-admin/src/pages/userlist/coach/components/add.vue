<template>
  ...
  >
    ...
    >
      ...
      <el-form-item label="教练介绍：" prop="intro" class="textInput">
        <div class="inputText">
          <el-input
      ...
      validator: (rule, value, callback) => {
        ...
        setTimeout(() => {
          if (isPhone.value === true) {
            callback(new Error("教练手机号已存在，请重新输入"));
          } else {
            callback();
          }
        }, 300);
      ...
      await addCoach(params)
        .then((res) => {
          if (res.code === 200) {
            ...
            nameNumVal.value = 0; //清空教练字数
            jobNumVal.value = 0; //清空岗位字数
            introNumVal.value = 0; //清空教练介绍字数
            handleClose();
            // 刷新列表
            emit("getList");
          } else {
...
const handleClose = () => {
  ...
  nameNumVal.value = 0; //教练字数显示
  jobNumVal.value = 0; //岗位字数显示
  introNumVal.value = 0; //教练介绍字数显示
  flag.value = false;
  emit("handleClose");
};
// 校验图片
