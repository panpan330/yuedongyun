...
<template>
  <div class="contentBox">
    ...
    <!-- end -->
    <div class="trainingTeachList">
      <div class="itemCon">
        <div class="headTitle">
          <span style="min-width: 80px">序号</span>
          <span style="min-width: 155px; margin-right: 20px">教练名称</span>
          <span style="min-width: 300px">教练介绍</span>
          <span
...
import { baseCoachesSave, getCoaches } from "@/api/program";
// 导入组件
// 新增按钮
...
let dialogVisible = ref(false); //选择教练弹层
let pic = ref(""); //要放大的图片
let coachId = ref(null);
const selectAdd = ref();
const text = ref("选择教练");
const deleteText = ref("此操作将该教练移出当前训练，是否继续？"); //删除提示
...
let selectData = reactive([]); //已经选择的教练
let dialogPicVisible = ref(false); //控制放大图片弹层显示隐藏
let trainingId = route.params.id; //训练id
// ------生命周期------
onMounted(() => {
  getList();
  getTrainingTearchList();
});
// ------定义方法------
// 获取教练列表值
const getList = async () => {
  loading.value = true;
  await getCoachList(searchData)
    .then((res) => {
      if (res.code === 200) {
        loading.value = false;
...
// 获取训练教练列表getCoachs
const getTrainingTearchList = async () => {
  loading.value = true;
  let params = {
    id: trainingId,
    see: false,
...
.trainingTeachList .itemCon .item li .con > div:nth-child(6) {
  min-width: 150px;
}
.trainingTeachList .itemCon .item li .con > div:nth-child(5) {
  min-width: 155px;
  padding-right: 30px;
}
.trainingTeachList .itemCon .item li .con > div:nth-child(4) {
  min-width: 155px;
}
.trainingTeachList .itemCon .item li .con > div:nth-child(3) {
  min-width: 300px;
}
.trainingTeachList .itemCon .item li .con > div:nth-child(2) {
  min-width: 155px;
  margin-right: 20px;
}
.trainingTeachList .itemCon .item li .con > div:nth-child(1) {
  min-width: 80px;
}
.trainingTeachList .itemCon .headTitle > span:nth-child(3),
.trainingTeachList .itemCon .headTitle > div:nth-child(3),
.trainingTeachList .itemCon .item li .con > span:nth-child(3),
.trainingTeachList .itemCon .item li .con > div:nth-child(3) {
  width: 300px;
}
.ellipsisHidden30 {
