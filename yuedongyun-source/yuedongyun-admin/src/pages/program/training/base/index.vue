...
<template>
  <el-form
    ...
    class="demo-ruleForm"
  >
    <el-form-item label="训练分类：" prop="thirdCateId">
    ...
    <el-form-item label="训练封面：" prop="coverUrl">
      <UploadImage
        @getFlag="getFlag"
    ...
    <el-form-item label="训练价格：" prop="price" v-if="fromData.free === 1">
    ...
    <el-form-item label="训练有效期：" prop="radio">
      <el-radio-group
    ...
    <el-form-item label="重点部位：" prop="trainPart">
      <div class="el-input">
        <el-input
          v-model="fromData.trainPart"
          placeholder="例如：全身燃脂 / 核心 / 下肢"
          :disabled="!fromData.canUpdate"
        />
      </div>
    </el-form-item>
    <el-form-item label="预计消耗：" prop="calorieBurn">
      <div class="el-input inputRight inputMarginR">
        <el-input
          type="number"
          v-model="fromData.calorieBurn"
          :disabled="!fromData.canUpdate"
          placeholder="请输入预计热量消耗"
          min="1"
        />
        <span class="text">kcal</span>
      </div>
    </el-form-item>
    <el-form-item label="训练介绍：" prop="introduce" class="textInput">
      <div class="inputText">
        <el-input
    ...
    <el-form-item label="适合人群：" prop="usePeople" class="textInput">
      <div class="inputText">
        <el-input
    ...
    <el-form-item
      label="教练介绍：&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp"
      class="textInput"
    >
      <div class="tearch">
        <div class="head">
        ...
        <div class="info">
          拥有多年体能训练与训练带练经验，擅长围绕减脂、塑形和基础力量训练制定分阶段计划，帮助会员建立稳定的训练习惯并持续跟进反馈。
        </div>
        <span class="tooltipIcon">
          <span class="hover">用户端将自动展示教练信息，无需手动设置</span>
        </span>
      </div>
    </el-form-item>
    <el-form-item label="训练详情：" prop="detail" class="textInput">
      <div class="inputText">
        <el-input
...
let introNumVal = ref(0); //训练介绍字数显示
let usePeopleNumVal = ref(0); //适宜人群字数显示
let detailNumVal = ref(0); //训练详情字数显示
let flag = ref(false); //用来控制上传图片是否校验成功
let flagTime = ref(false); //用来日期是否校验成功
let isTraining = ref(true); //用来区别是训练封面上传还是教练头像选择，因为用的是通用图片上传组件
let isTimeLimit = ref(false); // 用来判断有效期是限时还是永久，限时的时候显示
let price = ref(0); //金额
let trainingId = ref(null); //当前训练id
...
const validateUpload = (rule, value, callback) => {
  if (flag.value) {
    ...
    callback(new Error("训练封面为空，请上传训练封面"));
  }
};
// 训练名称校验
const validateName = async (rule, value, callback) => {
  if (value === "" || value === undefined) {
    callback(new Error("训练名称为空，请填写训练名称"));
  }
  let parent = {
    name: value,
    id: fromData.value.id,
  };
  await setTrainingsName(parent).then((res) => {
    if (res.code === 200) {
      const isexisted = res.data.existed;
      if (isexisted) {
        callback(new Error("训练名称重复，请重新输入"));
      } else {
        callback();
      }
    }
...
let getDetailData = async () => {
  await getTrainingsDetail(trainingId.value)
    .then((res) => {
      if (res.code === 200) {
...
const handleSubmit = async (str) => {
  ...
  if (valid) {
    ...
    let parent = {
      ...
      difficulty: fromData.value.difficulty,
      trainPart: fromData.value.trainPart,
      calorieBurn: Number(fromData.value.calorieBurn),
      thirdCateId:
    ...
    await baseInfoSave(parent)
      .then((res) => {
        if (res.code === 200) {
          ...
          trainingId.value = res.data.id;
          emit("getTrainingId", res.data.id);
          emit("getActive", 1);
          // 保存并返回
          if (str === "getback") {
            router.push({
            ...
            // 保存并下一步
            router.push({
              path: `/program/add/` + trainingId.value,
            });
          }
        } else {
          ElMessage({
