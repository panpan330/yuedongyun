...
let getDetailData = async () => {
  await getTrainingsDetail(trainingId)
    .then((res) => {
      if (res.code === 200) {
      ...
      await mediaUpload(val)
        .then((res) => {
          ...
          if (res.code === 200) {
            ...
            itemData.value.map((obj) => {
              obj.sessions.map((ele) => {
                if (sessionId.value === ele.id) {
...
.trainingList .el-collapse, .trainingList .el-collapse-item__wrap{
  min-width: 1065px;
}
:deep(.videoBox .el-dialog){
