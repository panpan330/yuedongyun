...
watch(props, () => {
  getDetailData();
  // console.log(props.trainingVideoData);
})
// ------定义方法------
// 获取详情
let getDetailData = () => {
  free.value = props.trainingVideoData
};

// 是否观看
const handleTrailer = (e, value) => {
  itemData.value.map((obj) => {
    obj.sessions.map((val) => {
      if (value.id === val.id) {
        val.trailer = e;
      }
    });
...
.trainingList .itemCon .headTitle > span:nth-child(2), .trainingList .itemCon .headTitle > div:nth-child(2), .trainingList .itemCon .item li .con > span:nth-child(2), .trainingList .itemCon .item li .con > div:nth-child(2){
  width: auto;
  min-width: 300px;
  }
