...
<template>
  <div>
    ...
    >
      ...
      <el-table-column label="训练名称" min-width="250" class-name="textLeft">
      ...
      <el-table-column label="训练难度" min-width="120">
        <template #default="scope">
          {{ formatDifficulty(scope.row.difficulty) }}
        </template>
      </el-table-column>
      <el-table-column prop="trainPart" label="重点部位" min-width="140">
      </el-table-column>
      <el-table-column label="预计消耗" min-width="130">
        <template #default="scope">
          {{ scope.row.calorieBurn || 0 }} kcal
        </template>
      </el-table-column>
      <el-table-column prop="sessions" label="训练节数" min-width="150">
      </el-table-column>
      <el-table-column
        prop="categories"
        label="所属训练分类"
...
// 上架下架弹出层
import TrainingStatus from "@/components/Trainingstatus/index.vue";
// 空页面
...
const deleteText = ref("此操作将永久删除该训练，是否继续？"); //删除提示
...
let trainingsesId = ref(""); //训练id
let trainingData = ref({}); //获取训练详情（下架弹层用）
...
const handleDescend = async () => {
  if (status.value === 1 || status.value === 3) {
    // 上架接口
    await baseUpShelf({ id: trainingsesId.value }).then((res) => {
      if (res.code === 200) {
        ElMessage({
    ...
    // 下架接口
    await baseDownShelf({ id: trainingsesId.value }).then((res) => {
      if (res.code === 200) {
        ElMessage({
...
const handleOpenDescend = async (row) => {
  ...
  if (time < newTime) {
    ...
    trainingsesId.value = row.id; //训练id
    status.value = row.status; //训练状态
    trainingData.value = row;
    ...
    if (props.status === 3) {
      ...
      if (row.purchaseEndTime) {
        ...
        if (status.value === 1) {
          ...
          title.value = "确认下架";
    statusText.value = "此操作将下架该训练，是否继续？";
        }
      }
    }
  }
...
const handleDelete = async () => {
  await deleteTrainings(trainingsesId.value)
    .then((res) => {
      if (res.code === 200) {
        handleClose();
        setTimeout(() => {
