    ...
    <div class="trainingList">
      <el-collapse accordion v-model="activeNames">
        <el-collapse-item v-for="(item, index) in itemData.value" :key="index">
          ...
          <div class="itemCon topicCon" v-if="item.sessions.length > 0">
            <div class="headTitle">
              <span>序号</span>
              <span>小节名称</span>
              <span>题目</span>
...
let assessmentData = ref([]); //保存小节或练习中的题目
    ...
    .then((res) => {
      if (res.code === 200) {
        ...
        jectData.value.map((value) => {
          let subIds = [];
          value.assessments.map((val) => {
            subIds.push(val.id);
          });
          value.assessmentIds = subIds;
          // obj = {
...
const saveAssessment = async () => {
  ...
  //     if (ele.assessmentIds && ele.assessmentIds.length > 0) {
  //       assessmentData.value.push(ele);
  //     }
  //   });
  // });
  // console.log(assessmentData.value);
  let params = {};
  if (assessmentData.value && assessmentData.value.length > 0) {
    assessmentData.value.map((value,index) => {
      // if (value.assessmentIds) {
      //   // obj.outlineId = value.id;
      //   obj = {
      //     outlineId: value.id,
      //     assessmentIds: value.assessmentIds,
      //   };
      //   arr.push(obj);
      // }
      ...
      if (jectData.value.length > 0) {
        jectData.value.map((val) => {
          delete val.assessments;
          if (value.outlineId === val.outlineId && value.assessmentIds) {
            val.assessmentIds = value.assessmentIds;
            assessmentData.value.splice(index,1)
            // obj = {
            //   outlineId: value.id,
            //   assessmentIds: value.assessmentIds,
            // };
          ...
          // console.log(val.outlineId)
          // arr2.push(val);
        });
      }
      
    });
    assessmentData.value = assessmentData.value.concat(jectData.value);
    params = {
      datas: assessmentData.value,
      id: trainingId,
    };
  } else {
    if (jectData.value.length > 0) {
  ...
  await baseAssessmentSave(params)
    .then((res) => {
      if (res.code === 200) {
        emit("getActive", 4);
        ElMessage({
...
const handleDelete = async () => {
  let knob = phaseItem.value; //要删除的小节
  // 删除阶段测试逻辑
  // 从服务器获取的数据没有修改的情况下删除
  if (jectData.value.length > 0) {
    jectData.value.map((val, index) => {
      if (val.outlineId === knob.id) {
        jectData.value.splice(index, 1);
      }
    });
  }
  // 设置了小节的题目或者添加了阶段考核情况下删除
  if (assessmentData.value.length > 0) {
    assessmentData.value.map((val, index) => {
      if (val.outlineId === knob.id) {
        assessmentData.value.splice(index, 1);
      }
    });
  ...
  itemData.value.map((val) => {
    val.sessions.map((ele) => {
      ...
      jectData.value.map((sub) => {
        if (ele.id === sub.outlineId && ele.id === obj.id) {
          jectIds.value = sub.assessmentIds;
        }
      });
      if (ele.id === obj.id && ele.assessmentIds && ele.assessmentIds !== undefined) {
        jectIds.value = ele.assessmentIds;
