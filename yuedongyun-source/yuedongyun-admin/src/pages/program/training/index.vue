      ...
      <div class="tableBox">
        <div class="conHead pad-30">
          ...
          <div class="tab">
            ...
            >
              <el-tab-pane
                v-for="(item, index) in trainingStatusData"
                :key="index"
                :label="item.label"
                :name="index"
              ></el-tab-pane>
...
// 公用数据
import { trainingStatusData } from "@/utils/commonData";
// 接口api
import { getTrainingsPage, getDetails } from "@/api/program";
// 导入组件
// 新增按钮
...
const getList = async () => {
  ...
  let parent = {};
  // 此处做了特殊处理
  // 训练分类和时间搜索默认的时一个数组，但是后端需要的分别是一级、二级、三级的id和时间字符串，组件还得有清除按钮，所以做了特殊处理，正常情况下不建议这么做
  ...
  let data = {
    ...
    trainPart: searchData.trainPart,
    keyword: searchData.keyword,
  };
  parent = {
    ...data,
  };
  if (categoryIdLv3.value && categoryIdLv3.value.length) {
    parent.firstCateId = categoryIdLv3.value[0];
    parent.secondCateId = categoryIdLv3.value[1];
    parent.thirdCateId = categoryIdLv3.value[2];
  }
  if (datePicker.value && datePicker.value.length) {
    parent.beginTime = datePicker.value[0];
    parent.endTime = datePicker.value[1];
  }

  await getTrainingsPage(parent)
    .then((res) => {
      if (res.code === 200) {
        loading.value = false;
...
const getDifficulty = (val) => {
  difficulty.value = val ?? null;
};
// 获取训练部位
const getTrainPart = (val) => {
  searchData.trainPart = val ?? null;
};
// 获取训练状态
const getStatus = (val) => {
  status.value = val;
