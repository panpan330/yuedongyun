  ...
  await getDetails(id)
    .then((res) => {
      if (res.code === 200) {
        ...
        // getTrainingOrderData(fromData.value.orderDetailId);
      }
    })
    .catch((err) => {});
};
// 获取训练订单明细信息
const getTrainingOrderData = async (id) => {
  await getTrainingOrderDetail(id)
    .then((res) => {
      if (res.code === 200) {
        fromTrainingData.value = res.data;
      }
    })
    .catch((err) => {});
};
