...
// 获取vuex存储数据
// 导入组件
import Coach from "./components/coach.vue" // 教练个人详情页
import Staffs from "./components/user.vue" // 用户个人详情页
// 接口api
import { getUserInfo, editCurrentUser } from "@/api/user.js"//教练接口
...
const coach = ref() //coach组件
// 定义一个布尔值，判断子组件的回调
const isEdit = ref(false)
const rulescoachpassWord = async () => {
  // 使用coach组件中的rulespassWord方法
  coach.value.rulespassWord()
}
// 教练基本信息
...
// 将子组件传过来的值赋值给coacheditData
const editCoach = (val) => {
  coacheditData.data = val
...
watch(userInfo, (val) => {
  if (userInfo.data.type == 1) {
    getCoachDetail() //userInfo.data.type == 1时，用户身份为教练，调用教练接口
...
const handleSaveCoach = async () => {
  ...
  if (isEdit.value == true) {
    ...
    // 当coacheditData.data中的intro、job、photo中任意一向和coachData.data中的intro、job、photo中任意一向不相等时，说明非密码区域内容有变化
    if (coacheditData.data.intro !== coachData.data.intro || coacheditData.data.job !== coachData.data.job || coacheditData.data.photo !== coachData.data.photo) {
      editCurrentUser(coacheditData.data)
        .then((res) => {
          if (res.code === 200) {
            ElMessage({
              message: "修改成功",
      ...
      // 当coacheditData.data中的password和oldPassword都有值，说明密码区域内容有变化执行修改操作
      if (coacheditData.data.oldPassword && coacheditData.data.password) {
        editCurrentUser(coacheditData.data)
          .then((res) => {
            if (res.code === 200) {
              ElMessage({
