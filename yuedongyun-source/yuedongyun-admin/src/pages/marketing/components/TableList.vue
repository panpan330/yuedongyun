...
<template>
  <div>
    ...
    <el-table :data="baseData" border stripe v-loading="loading">
      ...
      <el-table-column label="商城券名称" min-width="200" class-name="textLeft">
      ...
      <el-table-column prop="rule" label="商城券规则" min-width="320" />
      <el-table-column label="使用范围" prop="" min-width="150">
        <template #default="scope">
          {{ scope.row.specific ? "指定训练分类" : "全部训练分类" }}
        </template>
      </el-table-column>
      ...
      <el-table-column min-width="220" label="领取期限">
        <template #default="scope">
      ...
      <el-table-column min-width="220" label="有效期限">
        <template #default="scope">
          <div v-if="scope.row.termDays">
            领取后{{scope.row.termDays}}天有效
          </div>
...
const router = useRouter();
  const deleteText = ref("此操作将永久删除该商城券，是否继续？");
let redeem = ref(); //定义兑换码弹层的ref
let voucherId = ref(""); //优惠券id
...
const handleDelete = async () => {
  await deleteMarket(voucherId.value)
    .then((res) => {
      if (res.code === 200) {
        ElMessage({
...
const handleLookOpen = async (row) => {
  ...
  redeem.value.getList(voucherId.value); //通过ref刷新子组件的兑换码数据
