...
<template>
  <el-form
    ...
    class="demo-ruleForm"
  >
    <el-form-item label="商城券名称：" prop="name">
      <div class="el-input">
        <el-input
    ...
    <el-form-item label="使用范围：" prop="rangeType">
      <el-radio-group v-model="fromData.rangeType">
        <el-radio
          v-for="item in vouchersScopeData"
          :key="item.value"
          :label="item.value"
          >{{ item.label }}</el-radio
        >
    ...
    <el-form-item label="商城券类型：" prop="type">
      <el-radio-group v-model="fromData.type" @change="handleType">
        <el-radio
          v-for="item in vouchersTypeData"
          :key="item.value"
          :label="item.value"
          >{{ item.label }}</el-radio
        >
...
import { getDetails, saveMarket ,updateVoucher} from "@/api/marketing"
// 缓存
    ...
    .then((res) => {
      if (res.code === 200) {
        ...
        if(data.rangeType){
          ...
          // 遍历训练分类，与后端给的三级id做匹配，重新构建数据结构，回显在页面
          let arr = []
          scopes.forEach(s => {
...
const handleSubmit = async (str) => {
  ...
  if (valid) {
    ...
    let commitForm = parent.id ? updateVoucher : saveMarket;
