...
<template>
  <div class="cartsWrapper">
    ...
    <div class="container bg-wt marg-bt-20">
      <div class="title">我的购物车 <span>共{{carts.length}}项训练内容</span></div>
      <div class="tab"  v-if="carts.length > 0" >
        ...
        <div class="tabItem fx-sb" v-for="item in carts" :key="item.id">
          <div class="checkBox fx">
            ...
            <span class="name" @click="goDetails(item.trainingId)" style="cursor: pointer;">
              {{item.trainingName}}{{item.expired ? "(已失效)" : ""}}
            </span>
          </div>
      ...
      <div class="empty" v-else>
        <div><img src="@/assets/img_gouwuche.png" width="200" alt=""></div>
          <div class="desc">看到喜欢的训练内容，点击【加入购物车】，在这里统一下单</div>
...
const goSettlement = () => {
  ...
  router.push({path: '/pay/settlement', query: {trainingIds: list.map(c => c.trainingId).join()}})
}
</script>
