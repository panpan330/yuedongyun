<!-- 问答模块 -->
<template>
  <div class="trainingAsk bg-wt marg-bt-20">
...
import { getTrainingPhases, getAskList, delQuestions } from "@/api/trainingDetails.js"
...
const getTrainingPhasesData = async (id) => {
  await getTrainingPhases(id)
    .then((res) => {
      if (res.code == 200) {
        phaseData.value = [{id:'', index: '全部'},...res.data]
      } else {
        ElMessage({
          message:res.data.msg,
          type: 'error'
