...
<template>
  <div class="note bg-wt marg-bt-20">
    ...
    <AskPhaseItems :data="phaseData" @checkCahpter="checkCahpter"></AskPhaseItems>
    <div class="askCont">
...
import { getTrainingPhases } from "@/api/trainingDetails.js"
import { getAllNotes, delNote, notesGathers, unNotesGathers } from "@/api/notes.js"
import AskPhaseItems from "../../../components/AskPhaseItems.vue";
...
onMounted(() => {
  ...
  if(isLogin()){
    // 获取小节数据
    getTrainingPhasesData(route.query.id)
    // 获取问答列表
    getAskListsDataes()
  }
})
...
const checkCahpter = (id) => {
  ...
  params.value.sessionId = id
  getAskListsDataes()
}
// 获取笔记列表
const getAskListsDataes = async () => {
  params.value.sessionId == 'all' ?  params.value.sessionId = undefined : null
...
// 获取小节数据
const getTrainingPhasesData = async (id) => {
  await getTrainingPhases(id)
    .then((res) => {
      if (res.code == 200) {
        phaseData.value = [{id:'all', index: '全部'},...res.data]
      } else {
        ElMessage({
          message:res.data.msg,
          type: 'error'
