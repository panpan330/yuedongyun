    ...
    <div class="calendarCont">
      <div class="day" v-for="item in calendarData">
          <div class="taday" v-if="item.date==currentDay">
            ...
            <span v-else @click="fitpointsSignHandle(item)">打卡</span>
          </div>
...
import { getSignRecords } from "@/api/training.js";
...
// 定义emits
const emit = defineEmits(['fitpointsSign']);
...
// 打卡
const fitpointsSignHandle = (val) => {
  emit("fitpointsSign", val)
  val.isRecords = 1
}
// 获取打卡数据
const signData = ref([])
