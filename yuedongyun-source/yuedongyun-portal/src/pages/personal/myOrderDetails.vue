...
<template>
  <div class="myOrderDetailsWrapper ">
    ...
    <div class="content">
      <CardsTitle class="marg-bt-20" title="训练信息"/>
      <div>
        <el-table class="table" :data="orderDetails && orderDetails.details">
          <el-table-column prop="trainingName" center label="训练名称">
            <template #default="scope">
              <div>{{ scope.row.name }}</div>
            </template>
          </el-table-column>
          <el-table-column align="center" label="训练价格" width="180">
            <template #default="scope">
      ...
      <div class="info">
        ...
        <div class="fx">
          <span>商城券：</span>
          <span v-if="orderDetails && orderDetails.voucherRule.length" class="ruleBox">
            <span v-for="(r,i) in orderDetails.voucherRule" :key="i">
            {{ r }}
            </span>
          </span>
...
// 申请退款
const refundsOptions = ['由于技术问题无法正常开练', '未使用商城券', '个人原因（个人网络原因，计划有变，账号异常等）', '训练内容与预期不符', '其他']
...
const getOrderDetailsData = async () => {
  await getOrderDetails(route.query.id)
      .then((res) => {
        if (res.code === 200) {
          res.data.voucherRule = res.data.voucherDesc ? res.data.voucherDesc.split("/") : [];
