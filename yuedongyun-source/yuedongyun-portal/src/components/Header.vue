...
<template>
  <header class="bg-wt">
    <div class="container fx">
      ...
      <!-- 头部分类-start -->
      <div v-if="route.path != '/main/index' && route.path != '/login' " class="trainingClass font-bt2"
           @mouseover="() => isShow = true" @mouseout="() => isShow = false">
        <i class="iconfont zhy-icon_fenlei_nor"></i> 分类
      </div>
      <div v-if="route.path != '/main/index' && route.path != '/login' " class="trainingListPanel" v-show="isShow"
           @mouseover="() => isShow = true" @mouseout="() => isShow = false">
        <div class="firstItems">
          <TrainingCategory :data="trainingClass" type="float"></TrainingCategory>
        </div>
      </div>
      <!-- 头部分类-end -->
      ...
      <div class="fx-al-ct pt-rt">
        ...
        <div v-if="userInfo  && userInfo.name">
          <span class="marg-lr-40 font-bt2" style="padding:27px 0"
                 @click="() => {$router.push('/personal/main/myTraining')}" @mouseover="()=> workoutShow = true"
                 @mouseout="() => workoutShow = false">训练中心</span>
          <div  class="workoutCont" v-show="workoutShow && learnClassInfo && learnClassInfo.trainingAmount"
               @mouseover="()=> workoutShow = true" @mouseout="() => workoutShow = false">
            <div class="count"><em>{{ learnClassInfo && learnClassInfo.trainingAmount }}</em> 项训练内容</div>
            <div class="info" v-if="learnClassInfo &&learnClassInfo.trainingId">
              <div class="fx-sb">
                <span>正在训练：</span>
                <div class="fx">
                  <span class="bt"
                        @click="() => $router.push({path: '/workout/index', query: {id: learnClassInfo.trainingId}})">继续训练</span>
                  <span class="bt bt-grey1" @click="() => $router.push('/personal/main/myTraining')">全部训练</span>
                </div>
              </div>
              <div class="tit">{{ learnClassInfo && learnClassInfo.trainingName }}</div>
              <div class="perc fx-sb"> {{ learnClassInfo && learnClassInfo.latestSessionName }}
                <i>{{ learnClassInfo && (Math.round(learnClassInfo.learnedSessions * 100 / learnClassInfo.sessions)) }}%</i>
              </div>
            </div>
          </div>
        </div>
...
import {getMyWorkout, getTrainingCategorys} from '@/api/training.js'
...
const trainingClass = ref([]) // 分类数据
const isShow = ref(false)  // 分类展示
const learnClassInfo = ref(null) // 最近训练信息-训练中心展示
const workoutShow = ref(false) // 训练中心hover模块展示
...
onBeforeMount(async () => {
  ...
  trainingClass.value = dataCache.getTrainingClassDataes
  // 先从store里拿如何没有就请求分类信息获取
  if (trainingClass.value.length === 0) {
    getTrainingClassHandle()
  }
...
// 获取训练分类
const getTrainingClassHandle = async () => {
  await getTrainingCategorys()
      .then((res) => {
        if (res.code == 200) {
          trainingClass.value = res.data;
